...
# Po wczytaniu ramek danych przeprowadzam pare działań na ramkach danych
diff <- Df %>%
  filter(DepDelay > 0) %>%
  mutate(DiffDelay = DepDelay - ArrDelay) %>%
  select(TailNum, DiffDelay) 

diff_with_models <- merge(diff, Aircrafts, by.x = "TailNum", by.y = "tailnum") # dla nadgonień opóźnień
...
## DLA WYKRESU Z DEPDELAY I ARRDELAY W ZALEŻNOŚCI OD ROKU
